...
namespace Tobii.Research.Unity
{
    internal static class VRUtility
    {
        ...
        internal static Transform GetEyeTrackerOrigin(GameObject cam)
        {
            ...
            {
                zOffs = 0.0f;
                Debug.Log("We seem to not be using SteamVR. Do not assume any offset: " + zOffs);
            }
    ...
    internal static class VRWrap
    {
        ...
        private static System.Reflection.MethodInfo _getDeviceIndexMethod;
        private static System.Reflection.MethodInfo _getControllerStateMethod;
        private static System.Reflection.FieldInfo _buttonPressedField;
        private static object _controllerStateObject;
        private static uint _controllerIndexLeft = uint.MaxValue;
        private static uint _controllerIndexRight = uint.MaxValue;
        private static object[] _argumentList;
        ...
        private static uint GetControllerIndex(bool leftHand)
        {
            var cVRSystemType = CVRSystemType;

            if (cVRSystemType == null)
            {
                return uint.MaxValue;
            }

            var openVRSystem = OpenVRSystem;
            ...
            if (_getDeviceIndexMethod == null)
            {
                _getDeviceIndexMethod = cVRSystemType.GetMethod("GetTrackedDeviceIndexForControllerRole");
            ...
            return (uint)_getDeviceIndexMethod.Invoke(openVRSystem, new object[] { leftHand ? 1 : 2 });
        }

        internal static bool GetControllerPressed(uint controllerIndex, out bool padPressed, out bool triggerPressed)
        {
            padPressed = false;
            ...
            if (_getControllerStateMethod == null)
            {
                _getControllerStateMethod = cVRSystemType.GetMethod("GetControllerState");
            ...
            var controllerStateType = Type.GetType("Valve.VR.VRControllerState_t");

            if (controllerStateType == null)
            {
                return false;
            }

            if (_controllerStateObject == null)
            {
                _controllerStateObject = Activator.CreateInstance(controllerStateType);
            ...
            var controllerStateTypeSize = (uint)System.Runtime.InteropServices.Marshal.SizeOf(controllerStateType);

            if (_argumentList == null)
            {
                _argumentList = new object[] { controllerIndex, _controllerStateObject, controllerStateTypeSize };
            }

            _argumentList[0] = controllerIndex;

            if (!(bool)_getControllerStateMethod.Invoke(openVRSystem, _argumentList))
            {
                return false;
            }

            if (_buttonPressedField == null)
            {
                _buttonPressedField = controllerStateType.GetField("ulButtonPressed");
            ...
            var buttonPressed = (ulong)_buttonPressedField.GetValue(_argumentList[1]);

            // Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad == 32
            padPressed = (buttonPressed & (1UL << 32)) > 0L;

            // Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger == 33
            triggerPressed = (buttonPressed & (1UL << 33)) > 0L;

            return true;
        }

        internal static uint ControllerIndexLeft
        {
            get
            {
                if (_controllerIndexLeft == uint.MaxValue)
                {
                    _controllerIndexLeft = GetControllerIndex(leftHand: true);
                }

                return _controllerIndexLeft;
            }
        }

        internal static uint ControllerIndexRight
        {
            get
            {
                if (_controllerIndexRight == uint.MaxValue)
                {
                    _controllerIndexRight = GetControllerIndex(leftHand: false);
                }

                return _controllerIndexRight;
            }
        }
    }
}