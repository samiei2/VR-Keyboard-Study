...
public class GameManager : MonoBehaviour {
    ...
    public bool autoIncrementUserId;
...
"the picket line gives me the chills",
    ...
    private bool _inNextPhraseRoutine;
    ...
    private void ActiveKeyboard_KeyboardLayout_OnKeyPressed(object sender, KeyEventArgs args)
    {
        if (args.KeyId == KeyID.Enter)
        {
            ...
            if (!_inNextPhraseRoutine)
            {
                _inNextPhraseRoutine = true;
                StartCoroutine(ShowNextPhraseRoutine());
            }
            //ShowNextPhrase();
        }
    }

    private IEnumerator ShowNextPhraseRoutine()
    {
        ShowNextPhrase();
        yield return new WaitForSeconds(5);
        _inNextPhraseRoutine = false;
    }

    private void ShowNextPhrase()
    {
        if (currentSet != null && _inSession)
        {
            if (currentPhraseNumber < currentSet.Count - 1)
            {
                currentPhraseNumber++;
                ShowPhrase();
            }
