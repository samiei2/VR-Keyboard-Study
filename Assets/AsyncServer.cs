...
public class AsyncServer : MonoBehaviour
{
    ...
    private int _maxNumOfClients = -1;
    private int _port;
    ...
    public AsyncServer(int port, int dataSize, int maxNumOfClients) : this(port, dataSize)
    {
    ...
    /// <param name="port">Port on which to run server</param>
    /// <param name="dataSize">The size of each incoming message in bytes</param>
    public AsyncServer(int port, int dataSize) : this(port)
    {
        _dataSize = dataSize;
    }
    /// <summary>
    ...
    /// <param name="maxNumOfClients">Maximum number of clients</param>
    public AsyncServer(int port, List<char> messageDelimiters, int maxNumOfClients) : this(port, messageDelimiters)
    {
        _maxNumOfClients = maxNumOfClients;
    }
    /// <summary>
    ...
    /// <param name="port">Port on which to run server</param>
    /// <param name="messageDelimiters">List of characters to separate messages</param>
    public AsyncServer(int port, List<char> messageDelimiters) : this(port)
    {
    ...
    private void ClientRead(IAsyncResult iar)
    {
        ...
        try
        {
            ...
            listener.TcpClient.GetStream().Read(dataarr, 0, dataarr.Length);
            listener.Message = dataarr;
